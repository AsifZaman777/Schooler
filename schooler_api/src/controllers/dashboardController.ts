...
export const getDashboardStats = asyncHandler(async (req: Request, res: Response) => {
    ...
    const [
        ...
        Routine.countDocuments({
            status: 'active',
            dayOfWeek: today.toLocaleDateString('en-US', { weekday: 'lowercase' })
        }),
    ]);
...
export const getStudentEnrollmentRatio = asyncHandler(async (req: Request, res: Response) => {
...
export const getActiveStudentRatio = asyncHandler(async (req: Request, res: Response) => {
...
export const getAttendanceRatio = asyncHandler(async (req: Request, res: Response) => {
    const { startDate, endDate } = req.query;
    ...
    const result = Object.values(dailyStats).map((day: any) => ({
        date: day.date,
        attendanceRate: day.total > 0 ? ((day.present / day.total) * 100).toFixed(2) : 0,
        present: day.present,
        total: day.total,
    }));
...
export const getPaymentTable = asyncHandler(async (req: Request, res: Response) => {
...
export const getTodaysSchedule = asyncHandler(async (req: Request, res: Response) => {
    const today = new Date();
