...
export const getIncomeReport = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (groupBy === 'day') {
        groupByFormat = { $dateToString: { format: '%Y-%m-%d', date: '$paidDate' } };
...
export const getExpenseReport = asyncHandler(async (req: Request, res: Response) => {
    ...
    const totalExpense = expenseData.reduce((sum, item) => sum + item.totalExpense, 0);
...
export const getBusinessProjection = asyncHandler(async (req: Request, res: Response) => {
    ...
    const avgIncome = incomeHistory.reduce((sum, item) => sum + item.amount, 0) / incomeHistory.length || 0;
    const avgExpense = expenseHistory.reduce((sum, item) => sum + item.amount, 0) / expenseHistory.length || 0;
...
export const getProfitLossReport = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (startDate || endDate) {
        ...
        if (startDate) paymentFilter.paidDate.$gte = new Date(startDate as string);
    ...
    const monthlyData = await Payment.aggregate([
                ...
                pipeline: [
                    {
                        $match: {
                            $expr: {
                                $and: [
                                    { $eq: [{ $dateToString: { format: '%Y-%m', date: '$date' } }, '$$month'] },
                                    { $eq: ['$status', 'paid'] },
                                ],
                            },
                        },
        ...
        },
        {
            $project: {
                month: '$_id',
                income: 1,
                expense: { $ifNull: [{ $arrayElemAt: ['$expenseData.expense', 0] }, 0] },
                profit: {
                    $subtract: ['$income', { $ifNull: [{ $arrayElemAt: ['$expenseData.expense', 0] }, 0] }],
                },
            },
        },
        { $sort: { month: 1 } },
