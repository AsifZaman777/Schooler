...
export const bulkCreateAttendance = asyncHandler(async (req: Request, res: Response) => {
    ...
    const attendance = await Attendance.insertMany(attendanceData, { ordered: false });
...
export const getAttendanceByClassRoom = asyncHandler(async (req: Request, res: Response) => {
    ...
    const allStudents = await Student.find({ classRoomId, status: 'active' })
